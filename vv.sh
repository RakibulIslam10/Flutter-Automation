...
EOF
  ...
  route_name=$(camelCase "${capitalizedViewName}Screen")
  ...
  # Multi-line GetPage insertion
  route_code=$(printf "    GetPage(\n      name: Routes.%s,\n      page: () => const %sScreen(),\n      binding: %sBinding(),\n    )," "$route_name" "$capitalizedViewName" "$capitalizedViewName")
  sed -i "/\/\/Page Route List/a $route_code" "$page_file"

  echo "✅ View '$viewName' created with clean structure, camelCase route, binding, and widget part links"
done
